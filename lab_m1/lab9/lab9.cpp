...
#include <string>
#include <iostream>
#include <lab_m1/lab9/stb_image.h>
...
void Lab9::Init()
{
    ...
    {
        Shader* shader = new Shader("LabShader1");
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1, "lab9", "shaders", "VertexShader1.glsl"), GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(window->props.selfDir, SOURCE_PATH::M1, "lab9", "shaders", "FragmentShader1.glsl"), GL_FRAGMENT_SHADER);
        shader->CreateAndLink();
        shaders[shader->GetName()] = shader;
    }
}
...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-2, -0.5f, -3));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
        RenderSimpleMesh(meshes["plane"], shaders["LabShader1"], modelMatrix, mapTextures["heightmap"], mapTextures["ground"]);
    }
}
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    //heightPixels = stbi_load((textureID + "heightmap.png").c_str(), &width, &height, &channels, STBI_grey);
