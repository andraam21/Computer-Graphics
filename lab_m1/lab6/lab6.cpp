...
void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
    ...
    glUniform1f(time_location, Engine::GetElapsedTime());

    // TODO(student): Get shader location for uniform mat4 "Model"
    int model_location = glGetUniformLocation(shader->program, "Model");   

    /*time_location = glGetUniformLocation(shader->program, "b");
    glUniform3f(time_location, 1.0f, 2.0f, 0.0f);

    glm::vec3 p;
    glUniform3fv(time_location, 1, glm::value_ptr(p));*/
